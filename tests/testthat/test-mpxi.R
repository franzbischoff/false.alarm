...
tester0 <- mpx_stream_start(datar0, 80, 0.5, FALSE)
tester01 <- mpx_stream_start(datar01, 80, 0.5, FALSE)
tester02 <- mpx_stream_start(datar02, 80, 0.5, FALSE)
tester0100 <- mpx_stream_start(datar0100, 80, 0.5, FALSE)
testel0 <- mpx_stream_start(datal0, 80, 0.5, FALSE)
testel01 <- mpx_stream_start(datal01, 80, 0.5, FALSE)
testel02 <- mpx_stream_start(datal02, 80, 0.5, FALSE)
testel0100 <- mpx_stream_start(datal0100, 80, 0.5, FALSE)
...
test_that("Normal mpx + update batch right with external stats", {
  ...
  stats <- list(
    ...
    ddg = (data[seq.int(params$window_size + 1, data_len)] - msd$avg[seq.int(2, data_len - params$window_size + 1)]) + (data[seq.int(1, data_len - params$window_size)] - msd$avg[seq.int(1, data_len - params$window_size)])
  )

  data_vector <- seq.int(1, params$constraint)
  ...
  initial_mp <- mpx_stream_s_right(data[data_vector], batch_size = params$constraint, initial_mp, partial_stats, constraint = 0, progress = params$progress)
  test <- mpx_stream_start(data[data_vector], params$window_size, params$ez, params$progress)
...
test_that("Normal mpx + update batch right with external stats2", {
  ...
  expect_equal(initial_mp$offset, params$constraint)

  start <- initial_mp$offset - params$constraint + 1
  ...
  sec_mp <- mpx_stream_s_right(data[sec_data_vector], batch_size = params$batch, initial_mp, sec_stats, params$constraint, params$progress)
  test_vector <- seq.int(start + params$batch, end)
  test <- mpx_stream_start(data[test_vector], params$window_size, params$ez, params$progress)
  ...
  start <- sec_mp$offset - params$constraint + 1
  ...
  ter_mp <- mpx_stream_s_right(data[ter_data_vector], batch_size = params$batch, sec_mp, ter_stats, params$constraint, params$progress)
  test_vector <- seq.int(start + params$batch, end)
  test <- mpx_stream_start(data[test_vector], params$window_size, params$ez, params$progress)
...
test_that("Normal mpx + update batch right new constraint", {
  data <- as.numeric(tsmp::mp_fluss_data$tilt_abp$data)
  params <- list(
    window_size = 100,
    constraint = 5000,
  ...
  idxs <- seq.int(1, params$constraint)
  idxs_new <- seq.int(params$constraint + 1, params$constraint + params$batch)

  initial_mp <- mpx_stream_start(data[idxs], params$window_size, params$ez, FALSE)
  final_mp <- mpx_stream_start(data[(idxs + params$batch)], params$window_size, params$ez, FALSE)
  test_mp <- mpxi_rcpp(data[idxs_new], initial_mp, params$constraint, FALSE)
