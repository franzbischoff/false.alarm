...
From this first database's results, we can infer that the most interesting parameters to tune in the FLOSS algorithm are the `regime_threshold` and `regime_landmark`. The `window_size`, as the only parameter needed to compute the MP may be explored sparsely in other databases, and the `mp_threshold` and `time_constraint` can be removed.
...
```{r importanceshap3, eval=TRUE, echo=FALSE, message=FALSE, fig.height = 6, fig.width= 10, out.width="100%"}
...
```{r importanceshap4, eval=TRUE, echo=FALSE, message=FALSE, fig.height = 6, fig.width= 10, out.width="100%"}
#| fig.cap: "This shows the effect each variable has on the FLOSS score."
