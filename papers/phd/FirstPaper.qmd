...
scores1 <- scores_stats |>
  ...
  ggplot2::theme_bw() +
  ggplot2::labs(
    title = glue::glue("First dataset; {floor(mean(scores_stats$size))} models by recording"),
    colour = "IQR < 0.1", x = ggplot2::element_blank(), y = ggplot2::element_blank()
  )
```
...
scores2 <- scores_stats_mvds |>
  ...
  ggplot2::theme_bw() +
  ggplot2::labs(
    title = glue::glue("Second dataset; {floor(mean(scores_stats_mvds$size))} models by recording"),
    colour = "IQR < 0.1",
    x = ggplot2::element_blank(), y = "Score distribution"
  )
```
...
scores3 <- scores_stats_vtds |>
  ...
  ggplot2::theme_bw() +
  ggplot2::labs(
    title = glue::glue("Third dataset; {floor(mean(scores_stats_vtds$size))} models by recording"),
    colour = "IQR < 0.1",
    x = "Recording ID", y = ggplot2::element_blank()
  )
```

```{r fig-global, eval=TRUE, cache=FALSE, echo=FALSE, fig.height=6, fig.width=10, out.width="100%"}
...
scores1 <- scores1 +
  ...
  ) + ggplot2::coord_trans(y = "log10", ylim = c(0.00001, 10))
scores2 <- scores2 +
  ggplot2::scale_y_continuous(
    labels = scales::label_number(),
    breaks = scales::trans_breaks("log10", function(x) 10^x)
...
all <- (scores1 / scores2 / scores3) + plot_layout(guides = "collect")
all + plot_annotation(
  title = "Scores all",
  theme = ggplot2::theme_bw() + ggplot2::theme(
    plot.title = ggplot2::element_text(size = 20)
