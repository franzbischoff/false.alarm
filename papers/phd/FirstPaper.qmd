...
```{r fig-global, eval=TRUE, cache=TRUE, echo=FALSE, warning=FALSE, fig.height=6, fig.width=10, out.width="100%"}
...
@fig-bestmodels the performance of the three best models. They are ordered from left to right, from the worst record to the best record. The top model is the one with the lowest median across the scores. The red line indicates the median score. The scores above 10 are squished in the plot and colored according to the scale in the legend.
...
for (i in seq_len(length(best_models))) {
  ...
  plots[[i]] <- ggplot2::ggplot(dd, ggplot2::aes(x = reorder(record, -score), y = score, colour = score)) +
    ...
    ggplot2::scale_color_gradientn(colors = c("#ffd500db", "#ff8800", "#ff5e00", "#ff0000"), limits = c(10.1, 100)) +
...
if (file.exists(here("output", "scores_stats_model_3.rds"))) {
  scores_stats_model_mvds <- readRDS(here("output", "scores_stats_model_3.rds"))
} else {
  predictors_names_mvds <- c("regime_threshold", "window_size", "regime_landmark")
  scores_stats_model_mvds <- all_scores_mvds |>
    dplyr::group_by(across(all_of(predictors_names_mvds))) |>
    dplyr::mutate(model = glue::glue("{window_size}_{regime_threshold}_{regime_landmark}")) |>
    dplyr::reframe(
      record = record,
      model = model,
      score = score, min = min(score), q25 = quantile(score, 0.25),
  ...
  saveRDS(scores_stats_model_mvds, file = here::here("output", "scores_stats_model_3.rds"))
}

scores_stats_model_mvds$id <- (sprintf("%04d", (as.numeric(as.factor(scores_stats_model_mvds$model)))))
scores_stats_model_mvds$id_text <- (sprintf("Model_%04d", (as.numeric(as.factor(scores_stats_model_mvds$model)))))
scores_stats_model_mvds$record <- (sprintf("%02d", (as.numeric(factor(scores_stats_model_mvds$record, labels = levels(records_factors_mvds))))))
...
best_models_mvds <- scores_stats_model_mvds |>
  ...
  dplyr::pull(id_text) |>
  unique()
best_models_mvds <- best_models_mvds[1:3]
...
if (file.exists(here("output", "scores_stats_model_4.rds"))) {
  scores_stats_model_vtds <- readRDS(here("output", "scores_stats_model_4.rds"))
} else {
  predictors_names_vtds <- c("regime_threshold", "window_size", "regime_landmark")
  scores_stats_model_vtds <- all_scores_vtds |>
    dplyr::group_by(across(all_of(predictors_names_vtds))) |>
    dplyr::mutate(model = glue::glue("{window_size}_{regime_threshold}_{regime_landmark}")) |>
    dplyr::reframe(
      record = record,
      model = model,
  ...
  saveRDS(scores_stats_model_vtds, file = here::here("output", "scores_stats_model_4.rds"))
}

scores_stats_model_vtds$id <- (sprintf("%04d", (as.numeric(as.factor(scores_stats_model_vtds$model)))))
scores_stats_model_vtds$id_text <- (sprintf("Model_%04d", (as.numeric(as.factor(scores_stats_model_vtds$model)))))
scores_stats_model_vtds$record <- (sprintf("%02d", (as.numeric(factor(scores_stats_model_vtds$record, labels = levels(records_factors_vtds))))))
...
best_models_vtds <- scores_stats_model_vtds |>
  ...
  dplyr::pull(id_text) |>
  unique()
best_models_vtds <- best_models_vtds[1:3]
