...
&\quad + \left(window\_size \times regime\_landmark\right)
\end{aligned}
$$ {#eq-refitted}
...
@fig-interaction shows the variable interaction strength between pairs of variables in the first dataset, which from all datasets has the more difficult scenario (which is the presence or not of a synusal rythm). This dataset, shows us that after refitting the model, `window_size` and `regime_landmark` would be a great confounding factor if we did not take into account the interactions. `regime_threshold`/`mp_threshold` and `regime_landmark` also show a strong interaction that could lead to misinterpretation if not taken into account.
...
@fig-importance shows the variable importance using three methods for the first dataset. All methods agrees that `regime_threshold` is the most important parameter, followed by `regime_landmark`. The `window_size` is the least important parameter. The accentuating importance of `regime_threshold`, makes sense as it is the most evident parameter we need to set to determine if the *Arc Counts* are low enough to indicate a regime change.
...
@fig-importanceshap and @fig-importanceshap2 show the effect of each feature on the FLOSS score. The more evident information we can defer from both figures is the effect of `time_constraint` that initially is basically a flat line, conditioned by the nicely distributed `regime_threshold`. `window_size` follows a similar pattern and `mp_threshold` also has a flat line, behind of what seems some random noise, nevertheless suggests that keeping it at zero (the same as removing it), will not change the final result.
...
@fig-importance2 shows the parameters importances of the second dataset [@bischoff2023malig]. The parameter `regime_threshold` shows a clear importance on the correctness of the FLOSS. @fig-importanceshap3 shows the behavior of the parameters. We see that the `window_size` has a low bondary where the FLOSS score degrades (high values mean worse performance). The `regime_threshold` shows this same behavior. The `regime_landmark` has a more linear behavior as in the first dataset.
...
testing_data <- trained_model$testing_data
int <- readRDS(here("output", "importances_mvds.rds"))
...
@fig-importance3 shows the parameters importances of the third dataset [@bischoff2023vtachy]. The parameter `regime_threshold` still shows a clear importance on the correctness of the FLOSS. @fig-importanceshap4 shows behavior of the parameters on this dataset. Here we still see that the `regime_landmark` keeps a linear behavior, while `window_size` and `regime_threshold` show a mirrored pattern in comparison with the second dataset, nevertheless, `regime_threshold` resemble the pattern seen in the first dataset.
...
interactions <- readRDS(here::here("output", "importances_vtds.rds"))
importance_firm <- interactions$importance_firm
importance_perm <- interactions$importance_perm
importance_shap <- interactions$importance_shap
...
all_scores_mvds <- all_fitted_mvds |>
  tidyr::unnest(.predictions) |>
  dplyr::filter(regime_landmark < 10) |>
  dplyr::select(
    id, rep, .sizes, .id,
    all_of(predictors_names2),
    .config, .pred, truth
  ) |>
...
Based on your description and the plots:

In the top left plot for window_size, the color gradient indicates that as the window_size increases, the corresponding regime_threshold generally increases as well, moving from purple to yellow. This suggests a correlation between a larger window_size and a higher regime_threshold. The downward trend in Shapley values with increasing window_size implies that larger windows tend to improve the model's performance.

In the top right plot for regime_landmark, the dots are also colored based on regime_threshold. It seems that there is no clear pattern or gradient, which suggests that the regime_landmark does not have a strong interaction with regime_threshold in terms of affecting the Shapley value.

The bottom plot for regime_threshold uses window_size for coloring, from purple (low window_size) to yellow (high window_size). This plot shows that lower regime_threshold values tend to have higher (worse) Shapley values, but as regime_threshold increases, the performance improves, indicated by lower Shapley values. The color gradient shows that this effect is consistent across different window_size values.

Considering that low Shapley values indicate better performance, we can conclude that a model with a larger window_size and a higher regime_threshold tends to perform better. The effect of regime_landmark is less clear, as the Shapley values do not show a significant change across its range.

------------------------------------------------------------------------

The new image you uploaded, Figure 12, appears to be similar to the previous one, showing variable importance using three different methods, while Figure 13 seems to show the effect of these variables on the FLOSS score, akin to the lower plots in the previous image.

In Figure 12:

Feature Importance Ranking Measure using ICE curves (A): As with the previous figure, this plot shows regime_threshold having a higher importance than window_size and regime_landmark, with a similar pattern of variable importance. This reaffirms the previous interpretation that regime_threshold is a more influential variable in the model.

Permutation-based method (B): The importance of regime_threshold is again confirmed with a large error bar, indicating variability in the measurement. Window_size and regime_landmark show relatively lower importance and variability.

SHAP (C): This plot also indicates that regime_threshold has the highest importance, much greater than window_size and regime_landmark.

For Figure 13, the effects on the FLOSS score are as follows:

Shapley value vs. window_size: The left plot shows a nonlinear relationship, where the Shapley value decreases as window_size increases up to a point and then increases. This suggests that there is an optimal range of window_size values that contribute positively to the model's performance.

Shapley value vs. regime_landmark: The middle plot displays a broad distribution of Shapley values across the range of regime_landmark values, with no clear trend. The coloring by regime_threshold doesn't indicate a consistent pattern, which suggests that regime_landmark does not have a strong or consistent effect on the model's performance, or that its effect is highly dependent on the combination of regime_landmark with regime_threshold.

Shapley value vs. regime_threshold: The right plot shows a clear negative correlation between regime_threshold and the Shapley value, meaning that as regime_threshold increases, the model's performance improves significantly. The color gradient shows that this effect is consistent across different window_size values.

Overall, these plots confirm the previous interpretations: regime_threshold is a significant predictor of the model's performance, with a clear and consistent effect. The window_size seems to have an optimal range for performance, and regime_landmark does not exhibit a clear influence on the model's predictions. The interaction between regime_threshold and window_size appears to be an important factor in determining the model's performance.

""In conclusion, regime_threshold remains the most important variable influencing the model's performance, with a higher threshold generally leading to better results (lower Shapley values). The effect of window_size on Shapley values is now positive, suggesting a negative impact on model performance as window_size increases, which is a contrast to the previous figure's implication. Regime_landmark still shows no clear pattern, indicating a minimal or inconsistent effect on the FLOSS score.""
...
scores1 <- scores_stats |>
  dplyr::mutate(low_iqr = q75 - q25 < 10) |>
...
scores_stats_mvds$id <- sprintf("%02d", (as.numeric(records_factors_mvds)))
...
scores_stats_vtds$id <- sprintf("%02d", (as.numeric(records_factors_vtds)))
...
```{r fig-global, eval=TRUE, cache=FALSE, echo=FALSE, warning=FALSE, fig.height=6, fig.width=10, out.width="100%"}
#| fig-cap: "Violin plot showing the distribution of the FLOSS score achieved by all tested models by
#|  recording.  The left half shows the recordings that were difficult to predict, whereas the right half shows
...
#|  of less than 10.  As a simple approximation, if we have a recording with just one regime change, and the
#|  the model predicts exactly one change, about 10s far from the truth, the score will be roughly 1."
...
scores11 <- scores11 + ggplot2::scale_y_continuous(
  ...
  title = "Best", x = ggplot2::element_blank(), y = ggplot2::element_blank(), colour = "IQR < 10"
)
...
first_title <- glue::glue("First dataset; {floor(mean(scores_stats$size))} models by recording")
...
all + plot_layout(guides = "collect") + plot_annotation(
  ...
  caption = glue::glue("* Violin plot was clipped to {max_y2} for better visualization. The worst score reaches about {max_y}."),
  theme = ggplot2::theme_bw() + ggplot2::theme(
    plot.title = ggplot2::element_text(size = 20)
  )
)
