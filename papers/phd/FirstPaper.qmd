...
@fig-importanceshap3 shows the parameter importances of the second dataset [@bischoff2023malig]. Here we see that the `window_size` has a low bondary where the FLOSS score degrades (high values mean worse performance). The `regime_threshold` shows this same behavior. The `regime_landmark` has a more linear behavior as in the first dataset.
...
@fig-importanceshap4 shows the parameter importances of the third dataset [@bischoff2023vtachy]. Here we still see that the `regime_landmark` keeps a linear behavior, while `window_size` and `regime_threshold` show a mirrored pattern in comparison with the second dataset, nevertheless, `regime_threshold` resemble the pattern seen in the first dataset.
