...
-   Can we reduce the number of false alarms in the ICU setting?
-   Can we accomplish this objective using a minimalist approach (low CPU, low memory) while maintaining robustness?
...
-   The memory and space used to perform the primary goal of the algorithm (sound an alarm, for ex.) will be collected
    if available.
-   The type of algorithms used to identify ECG anomalies
-   The type of algorithms used to identify specific diagnoses (like a flutter, hyperkalemia, etc.)
...
-   Identify the obstacles of identifying life-threatening ECG changes within memory, space, and CPU constraints;
-   Be able to reduce ECG monitor's false alarms using the proposed methodology;
-   Assess the feasibility of implementing the algorithm in the real world and other settings than ICU monitors.

And outcomes:

-   To achieve a better score of false alarm reduction than the best on Physionet's 2015 challenge;
-   To push forward the state-of-the-art technology on false alarms reduction, maybe even being domain agnostic;
-   To draw more attention to fading factors as a reliable, fast, and cheap approximation of the true value;
-   To draw more attention to the matrix profile concept as an efficient, agnostic, and almost parameter-free way to
    analyze time series;
-   To draw more attention of the Patient Monitorization industry on solving the false alarm problem.
