...
as motif discovery, anomaly detection, regime change detection and others [@Yeh2017a].  The MP is
known to be an incredible fast algorithm [@gharghabi2018; @Madrid2019], thus viewing on the other
...
shapelets candidates.  The CP looks for patterns that are at the same time very *similar* to its
neighbors in class *A* while is very *different* from the nearest neighbor from class *B*.
...
Ultimately, this workflow will be experimented on a low power device, such as an ESP32 microcontroller [@esp32]
