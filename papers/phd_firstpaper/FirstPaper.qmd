...
rlang::check_installed(c(
  "here", "glue", "kableExtra", "ggplot2", "dplyr", "multidplyr", "vip", "pdp", "patchwork", "fastshap", "shapviz", "signal", "workflowr"
))
...
::: {#def-distprof}
: A distance profile $D \in \mathbb{R}^{n-m+1}$ of a time series $T$ and a subsequence $T_{i,m}$ is a vector that stores $\operatorname{dist}\left(T_{i, m}, T_{j, m}\right) \forall j \in [1, 2, \ldots, n − m + 1]$ , where $i \neq j$.
:::

Combining all distance profiles of a time series we get the matrix profile:

::: {#def-matrprof}
: A matrix profile $MP \in \mathbb{R}^{n-m+1}$ of a time series $T$ is a meta time series that stores the z-normalized Euclidean distance between each subsequence and its nearest neighbor (within the all subsequence set of $T$), where $n$ is the length of $T$ and $m$ is the given subsequence length.
:::
...
To evaluate the FLOSS algorithm performance under challenging conditions, we have chosen three datasets which include multiple patients with paroxysmal atrial fibrillation events, malignant ventricular ectopy and ventricular tachyarrhythmia[@bischoff2021afib; @bischoff2023malig; @bischoff2023vtachy]. The records were split in a proportion of approximately 3/4 for the training set (inner resampling) and 1/4 for the test set (outer resampling). The inner resampling was performed using a 5-fold cross-validation.
...
The dataset used was the "Paroxysmal Atrial Fibrillation Events Detection from Dynamic ECG Recordings: The 4th China Physiological Signal Challenge 2021" hosted by Zenodo [@bischoff2021afib] under the same license as Physionet. The selected records were those that contain paroxysmal atrial fibrillation events, a total of 229 records.
...
While this process is powerful and robust, it does not show us the importance of each parameter. This aspect is important for two reasons: (1) it allows us to reduce the search space for further datasets, and (2) it allows us to understand the impact of each parameter on the model.
...
After checking the interactions (later in @fig-interaction), this is the refitted model:
...
- **`time_constraint`**: is a parameter that must be interpreted cautiously.  The 0 (zero) value means **no constraint**, which is equivalent to the size of the FLOSS history buffer (in our setting, 5000). Since we only explored three values, what we may be seeing is cyclic behavior of the score. The size of the history buffer may be indeed related to the model performance, allowing the model to "forget" the past and focus on more recent data. As the exploration of this paramenter would be computationally too expensive, we will not explore it further and consider it as the size of the history bufffer.
...
@fig-importance2 shows the parameters importances of the second dataset [@bischoff2023malig]. The parameter `regime_threshold` shows now a clear importance on the correctness of the FLOSS. @fig-importanceshap3 shows the behavior of the parameters. We see that the `window_size` has a low boundary where the FLOSS score degrades (high SHAP values mean worse performance). The `regime_threshold` shows this same behavior as in the first dataset. The `regime_landmark` has a more linear behavior.
...
The blue color indicates the recordings with a small IQR (interquartile range) of model scores.

> TODO: explain the figure.
> Vemos que o primeiro dataset é realmente mais complicado que os outros 2
> Nos outros 2 vemos que vários conjuntos de parametros (= 1 modelo) conseguem ter um bom score
...
```{r fig-global, eval=TRUE, echo=FALSE, warning=FALSE, fig.height=6, fig.width=10, out.width="100%"}
...
> Talvez esta figura possa ser removida visto que a @fig-bestmodels (e demais) já mostra o desempenho de todos os datasets

> TODO: explain the figure.
> Aqui vemos que alguns modelos conseguiram um bom score na maioria dos conjuntos de dados
> Mostrar nos outros datasets também
> A legenda está errada, aparece duas vezes 'IQR < 0.5' e true para vermelho e azul. Precisa corrigir no codigo da figura
...
> TODO: preciso refazer esta tabela com os valores de score atualizados.
