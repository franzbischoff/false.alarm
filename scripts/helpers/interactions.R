#' Interaction effects - Borrowed from vip package
#'
...
#' fit <- gbm(y ~ .,
#'   data = trn, distribution = "gaussian", n.trees = 1000,
...
vint <- function(object, feature_names, data, type, n_jobs = 1, ...) {
  ...
  cli::cli_alert_info("Computing the importances of {length(all_pairs)} combinations.")
  ...
  progressr::with_progress(
    {
      p <- progressr::progressor(along = all_pairs)

      p("starting", amount = 0)
      # computing partials
      parts <- furrr::future_map(all_pairs, function(x, ...) {
        pd <- pdp::partial(object, pred.var = x, ...)
        p("computing partials")
        pd
      },
      train = data, type = type,
