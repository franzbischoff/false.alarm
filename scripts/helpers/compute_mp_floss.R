...
# FICA
floss_get_regimes <- function(ecg_data, window_size, mp_threshold, time_constraint,
...
compute_companion_stats <- function(ecg_data, params, infos) {
  ...
  msd <- matrixprofiler::muinvn(ecg_data, params$window_size, params$n_workers)
  avg <- msd$avg
  sig <- msd$sig
...
compute_s_profile_with_stats <- function(data_with_stats, params, infos) {
  ...
  current_mp <- matrixprofiler:::mpx_stream_s_right(data[initial_data_vector],
  ...
  profiles[[1L]]$motif_quality <- motif_quality(current_mp$right_matrix_profile,
  ...
  for (n in new_data_list) {
    ...
    current_mp <- matrixprofiler:::mpx_stream_s_right(data[data_vector],
...
motif_quality <- function(mp, method = c("mean", "median"), input_format = c("euclidean", "pearson"), window_size = NULL) {
  ...
  if (input_format == "pearson") {
    ...
    mp <- matrixprofiler::corr_ed(mp, window_size)
  }
