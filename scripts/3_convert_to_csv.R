...
# This script converts the FLOSS results from RDS format to CSV format
# compatible with the Python evaluation pipeline
...
test_mode <- FALSE # Set to FALSE to export all data
test_rows <- 1000 # Number of rows for test export

# Step 1 - Load FLOSS results
...
# Step 2 - Limit to test rows if in test mode
if (test_mode) {
...
csv_data <- data %>%
  mutate(
    ...
    # min_gap_samples is currently fixed at 200 in clean_pred()
    # TODO: This should become a grid search parameter in the future
    min_gap_samples = 200,

    # Convert duration from samples to seconds (250 Hz)
    duration_seconds = length / 250.0,

    # Convert ground truth indices to times (samples → seconds)
    gt_times = map(truth, function(x) if (is.null(x) || length(x) == 0) numeric(0) else x / 250.0),

    # Convert prediction indices to times (samples → seconds)
    det_times = map(pred, function(x) if (is.null(x) || length(x) == 0) numeric(0) else x / 250.0),

    # Count number of detections and ground truth events using lengths()
  ...
  ) # Step 4 - Convert list columns to JSON format (Python-compatible)
cli::cli_inform(c("i" = "Converting lists to JSON format"))

csv_data <- csv_data %>%
  mutate(
    # Convert R vectors to Python list strings [1.0, 2.0, 3.0]
...
# Basic validation - check for NAs and consistency
