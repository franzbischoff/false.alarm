render_floss_video <- function(video_file = here::here("dev", "floss_default.mp4"), ecg_data,
                               arc_counts, title = "Online Semantic Segmentation", framerate = 25, filter_w_size = 0, tail_clip = 0, temp_dir = "tmp", sample = FALSE) {
  done <- FALSE
  rate <- 250
  ten_secs <- 10 * rate # 10 seconds
  eighty_secs <- 80 * rate # 80 seconds
  ...
  if (!is.null(info$filter)) {
    filter <- tail(info$filter, ten_secs - filter_w_size + 1)
    event_offset <- sum(filter)
    event_line <- event_line + event_offset
  }

  event_line <- eighty_secs - event_line # 80 seconds minus the last 10 seconds
  ...
  diagnostic_limit_idx <- data_constraint - ten_secs # 10 seconds, for physionet
  ...
  for (d in seq_along(arc_counts)) {
    ...
    if (time_constraint > 0) {
      ...
      a <- a + ggplot2::annotate("segment", y = data_min, yend = data_max, x = curr_ts_constr, xend = curr_ts_constr, color = "red", size = 0.1) +
        ggplot2::annotate("text", x = curr_ts_constr - 40, y = data_min * 0.4, label = "Time constraint", color = "red", size = 1, angle = 90)
    }

    a <- a + ggplot2::theme_grey(base_size = 7) + ggplot2::theme(
    ...
    b <- ggplot2::ggplot(data.frame(x = cac_idxs, y = arc_counts[[d]]$cac), ggplot2::aes(x, y)) +
      ...
      ggplot2::annotate("text", x = cac_min_idx + 125, y = 0.4, label = sprintf("%d", cac_min_idx), color = "red", size = 1.5) +
