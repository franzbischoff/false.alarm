...
# Load all helper functions from the common directory
# This includes data reading, filtering, and preprocessing utilities
...
# Load core functions for FLOSS algorithm (Matrix Profile-based regime detection)
...
# var_signals_exclude <- setdiff(const_signals, var_signals_include)
#########
# Dataset configuration - AFib Regimes (Paroxysmal Atrial Fibrillation)
# This dataset contains ECG signals with regime changes in AFib episodes
...
# Resampling configuration (from 200Hz to 250Hz for standardization)
var_resample_from <- 200
var_resample_to <- const_sample_freq

# Data filtering options
var_subset <- NULL # 1:10000  # NULL = use entire signal
var_limit_per_class <- NULL # NULL = no limit on files per class

# Focus only on paroxysmal AFib (episodes that come and go)
var_classes_include <- "paroxysmal_afib"
var_classes_exclude <- setdiff(const_classes, var_classes_include)

# Use Lead II for analysis (most common for rhythm analysis)
var_signals_include <- "II"
var_signals_exclude <- setdiff(const_signals, var_signals_include)
########

# Step 1 - Find all ECG files matching the criteria
# Searches for regime-labeled files in the specified dataset directory
file_paths <- find_all_files(here::here("inst", "extdata", dataname),
  data_type = "regimes",
  classes = var_classes_include
  # limit_per_class = 10  # Uncomment to limit files for testing
)

# Step 2 - Read and prepare the data
# Loads ECG signals, resamples to 250Hz, normalizes (z-score), and extracts regime labels
dataset <- read_and_prepare_ecgs(file_paths,
  ...
  normalize = TRUE # Z-score normalization for better MP computation
)

# Step 3 - Transform dataset into tidy format (one row per file)
# Extract ground truth (regime change points) and time series data
tidy_dataset <- purrr::map_dfr(dataset, function(x) {
  regimes <- attr(x, "regimes") # Ground truth regime change positions
  if (length(regimes) == 0) {
    return(NULL) # Skip files with no regime changes in the subset
  }
  if (length(regimes) == 1 && regimes == 0) {
    return(NULL) # Skip files with only zero (no valid changes)
  }
  tibble::tibble(truth = list(regimes), ts = list(x[[var_signals_include]]))
}, .id = "id")

# Clean and validate ground truth positions
# Ensures regime positions are within signal bounds and properly formatted
for (i in seq_along(tidy_dataset$truth)) {
  tidy_dataset$truth[[i]] <- clean_truth(tidy_dataset$truth[[i]], length(tidy_dataset$ts[[i]]))
}

rm(dataset) # Free memory - no longer needed

#############
# Grid search hyperparameter - Matrix Profile window size
# Testing 3 values: 350, 375, 400 (approximately 1.4-1.6 seconds at 250Hz)
var_window_size <- seq(350, 400, by = 25)
split <- 5 # Split identifier for partial results saving
#############

tic <- Sys.time()
# Step 4 - Compute Matrix Profile using FLOSS algorithm
# Set up parallel processing for faster computation
future::plan(future::multicore) # Use all available CPU cores

# FLOSS (Fast Low-cost Online Semantic Segmentation) computation
# The Matrix Profile is computed in batches of 100 samples with a history buffer of 5000 samples
# This is memory-intensive - processing one window size at a time
floss_dataset <- NULL
for (w in var_window_size) {
  cli::cli_inform(c("i" = "Starting processing window size {w}"))
  tictic <- Sys.time()
  # Parallel computation of FLOSS for all time series
  # floss_train_regimes returns the arc curve needed for regime detection
  floss <- furrr::future_map(tidy_dataset$ts, ~ {
    floss_train_regimes(.x, w, 0, 0)
  }, .options = furrr::furrr_options(seed = NULL))
  checkmate::qassert(floss, "L+") # Validate output is a list
  floss_dataset <- dplyr::bind_rows(
    floss_dataset,
    tibble::add_column(tidy_dataset, window_size = w, floss = floss) |>
      dplyr::select(-ts) # Remove raw time series to save memory
  )
  tactac <- Sys.time()
...
rm(tidy_dataset) # Free memory - time series no longer needed

# Grid search hyperparameters for regime detection
# regime_threshold: sensitivity for detecting regime changes (18 values)
# regime_landmark: temporal lag (in seconds) where threshold is applied to arc curve (15 values)
#   - A landmark of 2 means the detection occurs 2 seconds behind the current streaming position
var_regime_threshold <- seq(0.05, 0.9, by = 0.05)
var_regime_landmark <- seq(2, 9, by = 0.5)

# Create complete grid: 18 thresholds × 15 landmarks = 270 combinations
# Combined with 3 window sizes = 810 models per record
grid <- expand.grid(
...
tic <- Sys.time()
# Step 5 - Generate predictions for all hyperparameter combinations
# For each FLOSS result, test all 270 (threshold × landmark) combinations
floss_preds <- purrr::map_dfr(seq_len(nrow(floss_dataset)), function(i) {
  ...
  tictic <- Sys.time()
  # Apply all grid combinations to this FLOSS result
  pred_dataset <- purrr::map_dfr(seq_len(nrow(grid)), function(j) {
    rt <- grid$regime_threshold[j]
    rl <- grid$regime_landmark[j]

    # floss_predict uses the arc curve to identify regime change points
    # clean_pred removes predictions too close to each other (within 200 samples), prioritize the first
    pred <- floss_predict(
      current_row$floss[[1]],
    ...
    # Store predictions with corresponding hyperparameters
    current_row |>
      dplyr::select(-floss) |> # Remove FLOSS object to save memory
      dplyr::mutate(
        regime_threshold = rt,
        regime_landmark = rl,
        pred = list(pred) # Predicted regime change positions
      )
  })
...
tic <- Sys.time()
# Step 6 - Compute evaluation scores
# score_pr is a custom F-score with temporal penalty
# Parameters: tolerance of 10s and 4s for matching predicted vs true regime changes
floss_scores <- list()
for (i in seq_len(nrow(floss_preds))) {
  score <- score_pr(floss_preds$truth[[i]], floss_preds$pred[[i]], const_sample_freq, 10, 4)
  floss_scores[[i]] <- 1 - score # Invert score (higher is better)
}

# Add scores to predictions and format hyperparameters
...
# Rename 'id' to 'record' for clarity and save partial results
...
# region Add Baseline
# Step 7 - Add baseline comparisons
# Baseline models predict regime changes at fixed intervals (naive approach)
# Reload the complete dataset (this assumes all splits were merged)
final_dataset <- readRDS(here::here("output", glue::glue("{dataname}.rds")))

# Rebuild tidy_dataset to get signal lengths (needed for baseline generation)
tidy_dataset <- dplyr::mutate(tidy_dataset, length = purrr::map_int(ts, length))

# Baseline 1 - Predict regime change every 1 second (250 samples at 250Hz)
# This represents a very aggressive detection strategy
...
# Baseline 2 - Predict regime change every 4 seconds (1000 samples at 250Hz)
# This is a more conservative detection strategy
...
# Join baseline predictions with the main dataset
# Each model's predictions will be compared against both baselines
final_dataset <- final_dataset |>
  dplyr::left_join(tidy_dataset |> dplyr::select(id, length, baseline, baseline4), by = c("record" = "id"))

# Compute baseline scores using the same metric as FLOSS models
# Baseline 1 score - fixed 1-second interval predictions
...
for (i in seq_len(nrow(final_dataset))) {
  ...
  baseline_scores[[i]] <- 1 - score # Invert for consistency (higher is better)
}

# Baseline 2 score - fixed 4-second interval predictions
baseline_scores4 <- list()
for (i in seq_len(nrow(final_dataset))) {
  score <- score_pr(final_dataset$truth[[i]], final_dataset$baseline4[[i]], const_sample_freq, 10, 4)
  baseline_scores4[[i]] <- 1 - score # Invert for consistency (higher is better)
}

# Add baseline scores to the dataset for comparison
...
# Save final dataset with FLOSS predictions and baseline comparisons
# This file will be used for cross-validation and comparison analysis
