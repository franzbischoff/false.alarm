...
[ ]Tiago: - Deve ser claramente determinado o objeto de investigação sobre o qual o candidato se pretende debruçar
[ ]jgama: - Research question - Must be proven
[ ]jgama: "(1) Can we reduce the number of false alarms in the ICU setting?" NOK, trivial
[ ]jgama: Rigor: the description of the methods
[ ]jgama: Rigor: and algorithms used
[ ]jgama: Rigor: the definition of the design of experiments
[ ]jgama: Rigor: and the definition of evaluation metrics.

[ ]jgama: The literature survey seems to be in a preliminary phase. The author selected the papers to read (mostrar o peer-review)
[ ]jgama: There is no related work section, could not understand the relevance of the roadmap Figures 4, 5, 6.
[ ]jgama: The main algorithms used or planned to use should be described or explained for example,the FLOSS algorithm, the Matrix Profile algorithm, etc. These algorithms should be described in a related work section.

[ ]jgama: The document is not self-contained
...
[ ]jgama: - The description of the method must be self-contained
...
[ ]Tiago: - Deve ser claramente estabelecido o plano de trabalho, especificadas as tarefas, e indicados os resultados esperados de cada uma.
[ ]Tiago: - Deve ainda ser apresentado uma proposta de publicações por forma a garantir os requisitos estipulados pelo ciclo de estudos.
[ ]Tiago: - atendendo à complexidade da temática em causa, o candidato deverá estabelecer um conjunto de tarefas cujo expectativa de sucesso de execução seja elevado, para conseguir cumprir os requisitos mínimos do programa.
...
as ward devices, home devices and, in the future, wearable devices.

The main questions are: (1) Can we accomplish this objective using a minimalist approach (low CPU,
low memory) while maintaining robustness?
...
The survey is being conducted with peer review, all articles on full-text phase were obtained and assessed for
the extraction phase, with exception of 5 articles that were not available. The survey is currently staled on
the Data Extraction phase due to external factors.

Fig. \@ref(fig:prisma) shows the flow diagram of the resulting screening using PRISMA format.

```{r prisma, echo=FALSE, out.width="50%", fig.cap="Flowchart of the literature survey."}
knitr::include_graphics("figure/PRISMA.png")
```

The peer review is being conducted by the author of this thesis together with another coleague,
Dr. Andrew Van Benschoten from Matrix Profile Foundation.

Table. \@ref(tab:kappa) shows the Inter-rater Reliability (IRR) using Cohen's kappa statistic.

```{r kappa, echo=FALSE, out.width="50%"}
knitr::include_graphics("figure/kappa.png")
```
...
The regime change approach will be using the _Arc Counts_.

, as explained  [@gharghabi2018].
