...
CSV format, allows more interoperability. Additionally, all the project, including the dataset is
...
has had great impact in any development lifecycle.
...
2021) and Fig. \@ref(fig:zenhub3) (as of Jan 2022). The literature survey is currently on the extraction
phase, that includes about 74 articles from the full-text screening.
...
```{r zenhub3, echo=FALSE, out.width="100%", fig.cap="Roadmap updated"}
knitr::include_graphics("figure/roadmap_updated.png")
...
By August 2021, it was clear that the chosen regime detection algorithm will be the (Fast Low-cost
...
Other algorithms we can cite are based on Hidden Markov Models (HMM) that requires at least two
...
discrete data. FLOSS demonstrated to be superior in several datasets in its original paper. In addition,
...
cluster similar sequences. Time Series Snippets exploits MPdist properties to summarize a dataset that
...
restructuration. The new roadmap was already shown above at Fig. \@ref(fig:zenhub3).
...
Fig. \@ref(fig:flowchart) shows the overview of the process.

```{r flowchart, echo=FALSE, out.width="100%", fig.cap="Regime changes with good but wandering data - true alarm"}
cat("@Bischoff2021a")
# knitr::include_graphics("figure/flowchart.png")
```
...
1. **On regime change detection:** in the original paper, the FLOSS algorithm assumes the _Arc
   ...
   comming from older data), and the _Arc Counts_ of newly incoming data are truncated by the same
   amount of temporal constraint. This prevents completely the detection of a regime change in the
   last 10 seconds as required. This issue is overcomed using the theoretical distributions as shown
   in Fig. \@ref(fig:distributions)

2. **On the Matrix Profile:** since the first paper presenting this new concept [@Yeh2017a], lots of
   investigation were made to speed up its computation. It is notable how all computations are not
   dependent on the _rolling window size_ as previous works not using Matrix Profile. Aside this, we
   ...
   1. The main issue on the algorithms above is the dependency on a fast Fourier transform (FFT)
      ...
      algorithm. We can argue that time complexity doens't mean "faster" when we can exploit low
      level instructions. In our case, using FFT in a low power device is overkilling. For example,
      a quick search over the internet gives us a hint that computing FFT on a 4096 data in an ESP32
      takes about 21ms (~47 computations in 1 second). This means ~79 seconds for computing all
      FFT's (~3797) required for STAMP using a window of 300. Currently we can compute 50k full
      matrices in less time using no FFT in an ESP32 MCU.
...
3. **On extending the Matrix Profile:** an unexplored constraint that we could apply on building the
   ...
   detecting pattern changes during time. The FLOSS algorithm rely on counting references between
...
The benchmarks of this implementation is available online [@Bischoff2021a].
