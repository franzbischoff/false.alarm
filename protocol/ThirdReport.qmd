...
format:
  pdf:
    ...
    extra-dependencies: ["svg", "flafter", "subfig"]
...
shown in @fig-targets. This package helps to keep record of the random seeds (allowing
...
where it was run. @fig-workflowr shows only a fraction of the generated website, where we
...
MP and PI respectively). We can have an idea of the relationship of both on @fig-thematrix.
...
For comparison, on @tbl-challenge we collected the score of the five best participants
...
The equation used on this challenge to compute the score of the algorithms is in the
@eq-score. This equation is the accuracy formula, with penalization of the false negatives.
...
@tbl-kappa shows the Inter-rater Reliability (IRR) of the screening phases, using Cohen's $\kappa$ statistic.
The bottom line shows the estimated accuracy after corrected for possible confounders [@Bakeman2011].

```{r tbl-kappa2, echo=FALSE, eval=FALSE}
...
#| label: tbl-kappa
#| tbl-cap: "Inter-rater Reliability on the literature survey process."
# knitr::is_latex_output()
cat(r"(
\begin{table}[ht]
\centering
...
"complexity" of the signal and correlates well with the noisy data is shown in @eq-complex [@Batista2014].
...
@fig-datafilter shows that noisy data (probably patient muscle movements) are marked
...
(MP)[@Yeh2017a], described on  @sec-matrixprofile. Recalling that the MP and the companion
...
@fig-arcsoriginal. This drop on the _Arc Counts_ is a signal that a change on the shape
...
The regime change detection will be evaluated following the criterias explained on
@sec-evaluation.
...
$\sqrt{2w}$. The @eq-contrast synthesizes this computation.
...
For a more complete understanding of the process, @fig-contrast shows a practical example
...
evaluated using the criterias explained on  @sec-evaluation.

### Summary of the methodology

In order to summarize the steps taken on this thesis to accomplish the main objective,
@fig-regimedetection, @fig-shapelets and @fig-fullmodel show the overview of the
processes involved.
...
@fig-nestedresampling shows us this principle. The steps (1) and (2) described above are
...
better interpreted. @fig-regimedetection starts with the "Full Dataset" that contains all
time series from the dataset described on  @sec-the-data. Each time series represents one
...
the evaluation will be performed and why the use of cross-validation will be explained on
@sec-evaluation.
...
@fig-shapelets shows the processes for training the classification model. First, the last
...
cross-validation will be explained on  @sec-evaluation.
...
Finally, @fig-fullmodel shows how the final model will be used on the field. In a
...
The author proposes the solution depicted in @fig-flosseval. It gives 0 as the best score
...
As described on @sec-classregime, the model for classification will use a set of shapelets
...
better, thus not suitable to our case. The F~1~ score is defined on  @eq-fscore.
...
doesn't have this issue [@Delgado2019]. MCC is defined on  @eq-mccval.
...
over time. It is defined on  @eq-kappam, where $p_0$ is the observed accuracy and
...
order to start the parameter tuning. An example of the current implementation is shown on @fig-flossregime.

```{r fig-flossregime, echo=FALSE, out.height="80%", out.width="80%"}
...
An example of candidates for ventricular tachycardia is presented on @fig-vtachy.

```{r fig-vtachy, echo=FALSE, out.width="90%", fig.height=9, fig.width=14}
#| fig-cap: "Shapelet candidates for Ventricular Tachycardia."
...
ESP32 MCU (@fig-esp32) in order to be sure if such small device can handle the task.
...
data in about 9 seconds in an ESP32 MCU (@fig-esp32), and keep updating it as fast as
...
are interchangeable using the @eq-edcc. However, we may argue that it is easier to
...
theoretical distribution using the temporal constraint parameters beforehand. as shown in
@fig-distributions. That gives us enough data to evaluate a regime change accurately utilizing
