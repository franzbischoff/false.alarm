...
@fig-thepipeline shows the workflow using Nested resamplig as described on @sec-methodology.
...
```{r fig-thepipeline, out.width="100%", fig.cap="FLOSS pipeline."}
if (knitr::is_latex_output()) {
...
@fig-interaction shows the variable interaction strength between pairs of variables.  That allows us to
verify if there are any significant interactions between the variables.  Using the information from the first model fit,
equation @eq-first, we see that `regime_threshold` interacts strongly with `regime_landmark`.  This interaction
was already expected, and we see that even after refitting the model, equation @eq-refitted, this interaction is
still strong.
...
@fig-importance then shows the variable importance using three methods: Feature Importance Ranking Measure
...
@fig-importanceshap and @fig-importanceshap2 show the effect of each feature on the FLOSS score.  The
...
@fig-global shows a violin plot of equal areas clipped to the minimum value. The blue color indicates the
...
@fig-worst shows the best effort in predicting the most complex recordings.  One information not declared
...
@fig-best shows the best performances of the best recordings. Notice that there are recordings with a
...
@fig-globalmodel shows the distribution of the FLOSS score of the 10% worst (left side) and 10% best models
...
@fig-bestmodels the performance of the six best models.  They are ordered from left to right, from the worst
