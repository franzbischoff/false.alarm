...
fluss_extract <- function(ecg_data, num_segments = 3, exclusion_zone = NULL) {
...
compute_arcs <- function(ecg_data, time_constraint = NULL, exclusion_zone = NULL) {
  checkmate::qassert(ecg_data, "L")
  checkmate::qassert(exclusion_zone, c("0", "N"))
  ...
  for (n in names) {
    ...
    if (is.null(time_constraint)) {
      ...
      mp$cac[max((profile_index_size - time_constraint + 1), 1):profile_index_size] <- 1.0
    }
