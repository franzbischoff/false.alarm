#' @export
contrast_error <- function(data, ...) {
  # cli::cli_alert(c("*" = "contrast_error <<- work here"))
  UseMethod("contrast_error")
}

contrast_error <- yardstick::new_numeric_metric(contrast_error, direction = "minimize")

#' @export
contrast_error_micro <- yardstick::metric_tweak("contrast_error_micro", contrast_error, estimator = "micro")

#' @export
contrast_error_macro <- yardstick::metric_tweak("contrast_error_macro", contrast_error, estimator = "macro")

#' @export
contrast_error.data.frame <- function(data, truth, estimate, na_rm = TRUE, estimator = "binary", case_weights = NULL, ...) {
  # nolint
  # cli::cli_alert(c("*" = "contrast_error.data.frame <<- work here"))
  # cli::cli_inform(c("*" = "contrast_error.data.frame: dots_n {rlang::dots_n(...)}"))
  if (rlang::dots_n(...) > 0L) {
    # 0L
    cli::cli_alert(c("*" = "contrast_error.data.frame: dots_names {names(rlang::dots_list(..., .preserve_empty = TRUE))}"))
  }

  "!DEBUG evaluating model."
  sizes <- rlang::expr(.sizes) # nolint
  yardstick::metric_summarizer(
    metric_nm = "contrast_error",
    metric_fn = contrast_error_vec,
    data = data,
...
contrast_error_vec <- function(truth, estimate, data_size, na_rm = TRUE, estimator = "binary", ...) {
  # cli::cli_alert(c("*" = "contrast_error_vec <<- work here"))
  # cli::cli_inform(c("*" = "contrast_error_vec: dots_n {rlang::dots_n(...)}"))
  if (rlang::dots_n(...) > 0L) {
    cli::cli_alert(c("*" = "contrast_error_vec: dots_names {names(rlang::dots_list(..., .preserve_empty = TRUE))}"))
  }
  ...
  contrast_error_impl <- function(truth, estimate, data_size, ...) {
    res <- contrast_score(truth, estimate, data_size)
    return(res)
  }
  ...
  if (estimator == "micro") {
    ...
    # cli::cli_inform(c("*" = "contrast_error_vec <<- micro"))
    res <- purrr::map2_dbl(
      truth, estimate,
      ~ yardstick::metric_vec_template(
        metric_impl = contrast_error_impl,
        truth = ..1,
        estimate = ..2,
        na_rm = na_rm,
        cls = "numeric",
    ...
    # cli::cli_inform(c("*" = "contrast_error_vec <<- macro"))
    res <- purrr::map2_dbl(
      truth, estimate,
      ~ yardstick::metric_vec_template(
        metric_impl = contrast_error_impl,
...
contrast_score <- function(gtruth, reported, data_size) {
