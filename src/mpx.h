...
List mpxis_rcpp(NumericVector data_ref, uint64_t batch_size, List object, List stats, uint64_t keep = 0,
                bool progress = false);
List mpxi_rcpp(NumericVector new_data, List object, uint64_t keep = 0, bool progress = false);
List mpxiright_rcpp(NumericVector data_ref, uint64_t window_size, double ez = 0.5, double s_size = 1.0,
                    bool idxs = true, bool euclidean = true, bool progress = false, uint64_t start = 1,
                    List old = R_NilValue);
