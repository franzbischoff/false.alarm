---
title: "Regime changes 5.0 - vtachyarrhythmias with PR"
...
if (!file.exists(here("output", "regime_outputs_vtds_pr.rds"))) {
  all_scores <- all_fitted %>%
    ...
    dplyr::mutate(score = fscore(truth, pred, size, window = 2500, beta = 2, alpha = 1, gamma = "one")) %>% # use this new score function
    tidyr::unnest(score) %>%
...
trained_model <- NULL
# Caching ===========
if (file.exists(here("output", "dbarts_fitted_lmk5.rds"))) {
  trained_model <- readRDS(here("output", "dbarts_fitted_lmk5.rds"))
} else {
  trained_model <- train_models(tree_data, parallel = TRUE, v = 5, rep = 1, grid = 30)
  saveRDS(trained_model, file = here("output", "dbarts_fitted_lmk5.rds"))
}
...
# Caching ===========
if (file.exists(here("output", "importances_lmk5.rds"))) {
  interactions <- readRDS(here("output", "importances_lmk5.rds"))
  ...
  ), file = here("output", "importances_lmk5.rds"))
}
...
scores_stats <- all_scores %>%
  dplyr::select(record, score, precision, recall) %>%
  # dplyr::filter(!is.na(score)) %>%
  dplyr::group_by(record) %>%
  dplyr::summarize(
    score = mean(score), min = min(score), median = median(score),
    q25 = quantile(score, 0.25), q75 = quantile(score, 0.75), precision = mean(precision), recall = mean(recall)
  ) %>%
  dplyr::ungroup()
...
scores_stats %>%
  ...
  dplyr::filter(min > quantile(min, 0.7) | min < quantile(min, 0.3)) %>%
...
```{r worst, eval=FALSE, fig.height=15, fig.width=12, out.width="100%", dev="png", cache=FALSE}
...
worst <- scores_stats %>%
  ...
  # dplyr::distinct(min, .keep_all = TRUE) %>%
  dplyr::slice_head(n = 10) %>%
  dplyr::pull(record)
...
```{r bestmodels, eval=TRUE, warning=FALSE, fig.height=22, fig.width=18, out.width="100%", cache=FALSE}
...
for (i in seq_len(length(best_models))) {
  ...
  plots[[i]] <- ggplot2::ggplot(ff, aes(x = record, y = 1 - score)) +
    ...
    ggplot2::geom_hline(aes(yintercept = median), colour = "#52046e", size = 0.5, linetype = 1) +
    ggplot2::geom_hline(aes(yintercept = mean), colour = "blue", size = 0.5, linetype = 2) +
...
# ggplot2::ggplot(ff, aes(x = record, xend = c(tail(record, -1), NA),
#  y = score, yend = c(tail(score, -1), NA))) +
#     ggplot2::geom_segment()
...
cli::cli_alert_info("RMSE: {yardstick::rmse_vec(train_x[, 6]$mean, predictions)}")
cli::cli_alert_info("RSQ: {yardstick::rsq_vec(train_x[, 6]$mean, predictions)}")
