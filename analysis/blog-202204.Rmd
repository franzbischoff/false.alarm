---
title: "2022, April Update"
author: "Francisco Bischoff"
date: "on April 17, 2022"
output:
...
if (file.exists(here::here("dev/backtofilters.rds"))) {
  backtofilters <- readRDS(here::here("dev/backtofilters.rds"))
  ...
  saveRDS(list(neg_files = neg_files, pos_files = pos_files, neg_filters = neg_filters, pos_filters = pos_filters),
    file = here::here("dev/backtofilters.rds"), compress = "xz"
  )
}
invisible(gc())
```
...
df_neg_all <- list_df(all_values_neg, TRUE, lag = 10, pad = 100)
df_neg_all <- df_neg_all[seq.int(1, nrow(df_neg_all), by = 5), ] # resize
...
df_pos_all <- list_df(all_values_pos, TRUE, lag = 10, pad = 100)
df_pos_all <- df_pos_all[seq.int(1, nrow(df_pos_all), by = 5), ] # resize
...
if (file.exists(here::here("dev/backtofilters2.rds"))) {
  backtofilters2 <- readRDS(here::here("dev/backtofilters2.rds"))
  ...
  filenames <- find_all_files(classes = "fibv", limit_per_class = max(neg, pos))
  ...
  saveRDS(list(neg_files = neg_files, pos_files = pos_files, neg_filters = neg_filters, pos_filters = pos_filters),
    file = here::here("dev/backtofilters2.rds"), compress = "xz"
  )
}
invisible(gc())
```
