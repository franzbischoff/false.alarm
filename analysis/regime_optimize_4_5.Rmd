...
if (!file.exists(here("output", "regime_outputs_vtds_pr.rds"))) {
  all_scores <- all_fitted %>%
    ...
    dplyr::mutate(score = fscore(truth, pred, size, window = 1250, beta = 3, alpha = 1, gamma = "one")) %>% # use this new score function
    tidyr::unnest(score) %>%
...
when the prediction is outside a window of 5 seconds (plus or minus 2.5s) from the labeled regime change.
