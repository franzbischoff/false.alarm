---
title: "Regime changes 4.5 - vtachyarrhythmias with PR"
...
train_models <- function(data, parallel = FALSE, v = 10, rep = 3, grid = 30, train = NULL, test = NULL) {
  ...
  wflw <- workflows::workflow() %>%
    ...
    workflows::add_formula(mean ~ regime_threshold + window_size + regime_threshold)
  ...
  return(list(model = final_flow, training_data = training_split, testing_data = testing_split, search = tune_search))
}
...
```{r buildoutputs}
if (!file.exists(here("output", "regime_outputs_vtds.rds"))) {
  ...
  saveRDS(all_fitted, file = here("output", "regime_outputs_vtds.rds"))
  rm("all_objs")
  rm("all_fitted")
}
```
...
network <- readRDS(here::here("output", "regime_network_vtds.rds"))
all_fitted <- readRDS(here::here("output", "regime_outputs_vtds.rds"))
net <- network %>%
...
fscore <- function(truth, pred, size, window = 1000, delta_prec = "flat", delta_rec = "flat",
                   gamma = "one", alpha = 1, beta = 1) {
  score <- score_regimes_precision(truth, pred, size,
    window = window, delta_prec = delta_prec,
    delta_rec = delta_rec, gamma = gamma, alpha = alpha,
    beta = beta
  )
  score <- dplyr::bind_rows(score)
  score$fscore[is.na(score$fscore)] <- 0
  result <- pmax(1 - score$fscore, 0)
  return(tibble::tibble(precision = score$precision, recall = score$recall, fscore = result))
}

if (!file.exists(here("output", "regime_outputs_vtds_pr.rds"))) {
  all_scores <- all_fitted %>%
    # dplyr::slice_head(n = 10) %>%
    tidyr::unnest(.predictions) %>%
    dplyr::filter(regime_landmark < 10) %>% # ten is too much
    dplyr::select(
      id, rep, .sizes, .id,
    ...
    dplyr::mutate(score = fscore(truth, pred, size, window = 2500, beta = 3, alpha = 1, gamma = "one")) %>% # use this new score function
    tidyr::unnest(score) %>%
    dplyr::rename(score = fscore)

  saveRDS(all_scores, file = here("output", "regime_outputs_vtds_pr.rds"))
} else {
  all_scores <- readRDS(here::here("output", "regime_outputs_vtds_pr.rds"))
}
```
...
models in another dataset, the "CU Ventricular Tachyarrhythmia Database" which contains 35 eight-minute ECG recordings
of human subjects who experienced episodes of sustained ventricular tachycardia, ventricular flutter, and ventricular
fibrillation.

This version of the analysis uses another scoring function, which is based on the F-score, and uses a simple approach
to calculate the Precision and Recall. The Recall reflects the hability of the model to detect the labeled regime changes;
the algorithm accounts as a "detection" if the prediction is within a window of 5 seconds (plus or minus 2.5s).
The Precision reflects the hability of the model to avoid false positives; the algorithm accounts as a "false positive"
when the prediction is outside a window of 10 seconds (plus or minus 5s) from the labeled regime change.

The F-score will be calculated giving more weight to the Recall (beta = 3).
Except for the Fig. \@ref(fig:bestmodels), all other figures will represent the F-score as $Score = 1 - F_{score}$, to make
it easier to compare with the previous score function.
...
if there are any significant interactions between the variables. Here we see that the interaction between variables are
minimal.

```{r modelbart, message=FALSE, cache=FALSE}
tree_data <- all_scores %>%
  dplyr::group_by(across(all_of(predictors_names))) %>%
  dplyr::summarize(mean = mean(score)) %>% # , prec = mean(precision), rec = mean(recall)) %>%
  dplyr::ungroup()
...
trained_model <- NULL
# Caching ===========
if (file.exists(here("output", "dbarts_fitted_vtds_pr.rds"))) {
  trained_model <- readRDS(here("output", "dbarts_fitted_vtds_pr.rds"))
} else {
  trained_model <- train_models(tree_data, parallel = TRUE, v = 5, rep = 1, grid = 30)
  saveRDS(trained_model, file = here("output", "dbarts_fitted_vtds_pr.rds"))
}
...
# Caching ===========
if (file.exists(here("output", "importances_vtds_pr.rds"))) {
  interactions <- readRDS(here("output", "importances_vtds_pr.rds"))
  ...
  ), file = here("output", "importances_vtds_pr.rds"))
}
```
...
#|  Shows almost no interaction between the variables."
...
After evaluating the interactions, we can then perform the analysis of the variable importance. The goal is to
understand how the FLOSS score behaves when we change the parameters.
...
- **`regime_landmark`**: seems to have lost its importance using this metric.

- **`window_size`**: has some importance, and as before, value below 75 starts to degrade the FLOSS score.
...
a1 <- shapviz::sv_dependence(d1, "window_size", color_var = "regime_threshold") +
  ggplot2::geom_smooth(method = loess, colour = "#0000ff44", alpha = 0.2) +
  ggplot2::labs(y = ggplot2::element_blank()) +
  ggplot2::theme_bw()
a2 <- shapviz::sv_dependence(d1, "regime_threshold", color_var = "regime_landmark") +
  ggplot2::geom_smooth(method = loess, colour = "#0000ff44", alpha = 0.2) +
  ggplot2::labs(y = ggplot2::element_blank()) +
  ggplot2::theme_bw()
...
#|  recordings where models had an IQR variability of less than 0.5."
...
```{r worst, eval=TRUE, fig.height=9, fig.width=12, out.width="100%", dev="png", cache=FALSE}
#| fig.cap="Prediction of the worst recordings (red is the truth, blue are the predictions)."

worst <- scores_stats %>%
  dplyr::filter(min > quantile(min, 0.8)) %>%
  dplyr::group_by(record) %>%
  dplyr::slice_head() %>%
  dplyr::ungroup() %>%
...
#| fig.cap="Prediction of the best recordings (red is the truth, blue are the predictions)."

bests <- scores_stats %>%
  dplyr::filter(min < quantile(min, 0.2)) %>%
  dplyr::group_by(record) %>%
  dplyr::slice_head() %>%
  dplyr::ungroup() %>%
  dplyr::arrange(desc(min), desc(median)) %>%
...
[Shiny app](https://franzbischoff.shinyapps.io/FLOSS_vtachy/) (default Score).

### By model

Fig. \@ref(fig:globalmodel) shows the distribution of the FLOSS score of the 25% worst (left side) and 25% best models
across the recordings (right side). The bluish color highlights the models with SD below 0.5 and IQR below 0.5.
...
#|  inner ressample.  The left half shows the models with the worst performances (top 25), whereas
#|  the right half shows the models with the best performances (top 25).
#|  The models are sorted (left-right) by the mean score (top) and by the median (below). Ties are
#|  sorted by the SD and IQR, respectively.  The bluish colors highlights models with an SD below 0.5
#|  and IQR below 0.5."
...
model_mean <- scores_stats_model %>%
  dplyr::mutate(low_sd = sd < 0.5) %>%
  dplyr::filter(id %in% c(worst_mean, best_mean)) %>%
  ...
  ggplot2::labs(subtitle = "Ordered by Mean and SD", colour = "SD < 0.5", x = ggplot2::element_blank(), y = "Score distribution")
...
model_median <- scores_stats_model %>%
  dplyr::mutate(low_iqr = q75 - q25 < 0.5) %>%
  dplyr::filter(id %in% c(worst_median, best_median)) %>%
  ...
  ggplot2::theme_bw() +
  ggplot2::theme(axis.text.x = element_text(size = 8, angle = 90, vjust = 0.5, hjust = 1)) +
  ggplot2::labs(subtitle = "Ordered by Median and IQR", colour = "IQR < 0.5", x = "Model ID", y = "Score distribution")

(model_mean / model_median) + plot_layout(guides = "auto") +
  plot_annotation(
    title = "Scores grouped by model (lower is better)",
    theme = ggplot2::theme_bw()
  )
```

Fig.  \@ref(fig:bestmodels) the performance of the six best models.  They are ordered from left to right, from the best
record to the worst record.  The top model is the one with the highest mean across the scores.  The blue line indicates
the precision of the model, and the green line the recall. The black line with dots is the f-score. The red continuous
and dashed lines are the median and the mean f-score respectively. The gray line limits the zero-score region.

```{r bestmodels, eval=TRUE, warning=FALSE, fig.height=18, fig.width=18, out.width="100%", cache=FALSE}
#| fig.cap="Performances of the best 6 models across all inner resample of recordings.
#|  The recordings are ordered by score, from the best to the worst.
#|  Each plot shows one model, starting from the best one.
#|  The blue line indicates the precision of the model. The green line the recall. The black line with dots is the f-score.
#|  The red continuous and dashed lines indicate respectively the median and mean f-score of the model.
...
for (i in seq_len(length(best_models))) {
  ...
  ff <- dd %>% dplyr::arrange(score, desc(recall))
  ff$record <- factor(ff$record, levels = ff$record, ordered = TRUE)

  plots[[i]] <- ggplot2::ggplot(ff, aes(x = record, y = 1 - score)) +
    ggplot2::geom_point(size = 2, colour = "#00000080") +
    ggplot2::geom_segment(aes(xend = c(tail(record, -1), NA), yend = c(tail(1 - score, -1), NA))) +
    ggplot2::geom_segment(aes(
      xend = c(tail(record, -1), NA), y = precision,
      yend = c(tail(precision, -1), NA)
    ), colour = "#008cff", size = 1) +
    ggplot2::geom_segment(aes(
      xend = c(tail(record, -1), NA), y = recall,
      yend = c(tail(recall, -1), NA)
    ), colour = "#26ff00", size = 1) +
    ggplot2::geom_hline(aes(yintercept = 1 - median), colour = "red", size = 0.5, linetype = 1) +
    ggplot2::geom_hline(aes(yintercept = 1 - mean), colour = "red", size = 0.5, linetype = 2) +
