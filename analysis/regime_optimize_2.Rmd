...
# saveRDS(all_scores_2, here::here("output/regime_outputs_lmk.rds"))
```
...
if (file.exists(here("output", "importances_lmk.rds"))) {
  ...
  set.seed(102)
  trdata <- dplyr::slice_sample(train_data, n = 2000)
  ttdata <- dplyr::slice_sample(testing_data[, predictors_names], n = 2000)
  ...
  set.seed(102)
  trdata <- dplyr::slice_sample(train_data[, predictors_names], n = 2000)
  ttdata <- dplyr::slice_sample(testing_data[, predictors_names], n = 2000)
  shap_fastshap_all_test <- shap_explain(best_fit, trdata,
    ttdata, predictors_names,
...
predictors_names_int <- c(predictors_names, "int_rt_rl", "int_mp_w", "int_mp_rl")
...
if (file.exists(here("output", "importances2_lmk.rds"))) {
  ...
  # saveRDS(interactions2, file = here("tmp", "interactions2xxx.rds"))
  ...
  # saveRDS(importance_firm2, file = here("tmp", "importancefirm2xxx.rds"))
  # importance_firm2 <- readRDS(here("tmp", "importancefirm2xxx.rds"))
  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking permutation importances...")
  importance_perm2 <- check_importance(best_fit2, testing_data2, testing_data2, predictors_names_int,
    type = "permute",
    nsim = 100, parallel = TRUE
  )
  ...
  importance_perm2 <- tibble::as_tibble(t(importance_perm2)) %>%
    dplyr::select(all_of(predictors_names)) %>%
    tidyr::pivot_longer(everything(), names_to = "Variable", values_to = "Importance")
  # saveRDS(importance_perm2, file = here("tmp", "importanceperm2xxx.rds"))
  # importance_perm2 <- readRDS(here("tmp", "importanceperm2xxx.rds"))
  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking SHAP importances...")
  set.seed(102)
  trdata2 <- dplyr::slice_sample(train_data2[, predictors_names_int], n = 2000)
  ttdata2 <- dplyr::slice_sample(testing_data2[, predictors_names_int], n = 2000)
  ...
  # importance_shap2 <- readRDS(here("tmp", "importanceshap2xxx.rds"))
  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking SHAP explanations...")

  set.seed(102)
  trdata2 <- dplyr::slice_sample(train_data2[, predictors_names_int], n = 2000)
  ttdata2 <- dplyr::slice_sample(testing_data2[, predictors_names_int], n = 2000)
  shap_fastshap_all_test2 <- shap_explain(best_fit2, trdata2,
    ttdata2, predictors_names,
    nsim = 100, parallel = TRUE
  )
  # saveRDS(shap_fastshap_all_test2, file = here("tmp", "shapfastshapalltest2xxx.rds"))
  ...
  shap_html_test2 <- fastshap::force_plot(
    ...
    feature_values = ttdata2[, predictors_names], baseline = mean(preds_test2$.pred), display = "html"
  )
...
set.seed(102)
ttdata <- dplyr::slice_sample(testing_data, n = 2000)

d1 <- shapviz::shapviz(shap_fastshap_all_test, X = ttdata[, predictors_names], baseline = mean(ttdata[, outcome_name]$mean))
...
set.seed(102)
ttdata2 <- dplyr::slice_sample(testing_data2, n = 2000)

d1 <- shapviz::shapviz(shap_fastshap_all_test2, X = ttdata2[, predictors_names], baseline = mean(ttdata2[, outcome_name]$mean))
a1 <- shapviz::sv_dependence(d1, "window_size", color_var = "auto") +
  ggplot2::geom_smooth(method = loess, colour = "#0000ff44", alpha = 0.2) +
  ggplot2::labs(y = ggplot2::element_blank()) +
  ggplot2::theme_bw()
...
# / model_median
(model_mean) + plot_layout(guides = "auto") +
  plot_annotation(
