...
check_interactions <- function(model, train_data, features, parallel = FALSE) {
  ...
  jobs <- 1
  if (parallel) {
    jobs <- parallelly::availableCores()
  }
  # Quantify relative interaction strength
  # set.seed(2022)
  # cut down the number of samples to speed up the process because of the number of features
  # if (nrow(train_data) > 2500 && length(features) > 3) {
  #   train_data <- dplyr::slice_sample(train_data, n = 2000)
  # }
  interact <- suppressWarnings(vint(model$fit$fit,
    type = "regression", n_jobs = jobs,
...
# }
# scores <- mdplyr(all_scores, vars = c("score_regimes_weighted"))
# scores <- scores %>% dplyr::mutate(score = score_regimes_weighted(truth, pred, 250, 10))
# all_scores2 <- scores %>% dplyr::collect()
# saveRDS(all_scores2, here::here("output", "regime_outputs_lmk.rds"))

# If needs recompute the scores
# all_scores <- all_scores %>%
# dplyr::mutate(score = score_regimes_weighted(truth, pred, 250, 10))

#####################
# This is how we added the "truth" column to the all_scores_1
# recs <- sort(unique(all_scores$record))
# regimes <- list()
# for (i in recs) {
#   ecg <- read_ecg_with_atr(here::here("inst", "extdata", "afib_regimes", i),
#     resample_from = 200,
#     resample_to = 250,
#     normalize = TRUE
#   )
#   size <- length(ecg[[1]]$time)
#   reg <- attr(ecg[[1]], "regimes")
#   regimes[[i]] <- clean_truth(reg, size)
# }
# truth <- list()
# for (i in seq_along(all_scores$record)) {
#   truth[[i]] <- regimes[[all_scores$record[i]]]
# }
# all_scores$truth <- truth
# saveRDS(all_scores, here::here("output", "regime_outputs_lmk.rds"), compress = "xz")
```
...
&\quad + mp\_threshold + window\_size\\
&\quad + regime\_threshold + regime\_landmark\\
&\quad + \left(time\_constraint \times regime\_landmark\right)\\
...
equation \@ref(eq:first), we see that `time_constraint` interacts strongly with `regime_landmark` and `regime_threshold`.
Also it seems that `regime_landmark` and `regime_threshold` interacts strongly with each other.
...
In order to explore the interactions, we chose to refit the model adding the first two interactions, equation \@ref(eq:refitted).
Here we see at the lower plot of Fig. \@ref(fig:interaction) that accounting for these interactions, the global strength of
the variables visibly changes, with `regime_landmark` and `regime_threshold` losing strength.

```{r modelbart, message=FALSE, warning=FALSE, cache=FALSE}
...
# pred1 <- predict(best_fit, testing_data)$.pred
# pred2 <- predict(best_fit, train_data)$.pred
# yardstick::huber_loss_vec(testing_data$mean, pred1) # 0.332674
# yardstick::rmse_vec(testing_data$mean, pred1) # 1.030549 (2-3%)
# yardstick::rsq_vec(testing_data$mean, pred1) # 0.9936
# yardstick::huber_loss_vec(train_data$mean, pred2) # 0.27744
# yardstick::rmse_vec(train_data$mean, pred2) # 0.8935
# yardstick::rsq_vec(train_data$mean, pred2) # 0.9952196
...
if (file.exists(here("output", "importances_lmk.rds"))) {
  ...
  interactions <- check_interactions(best_fit, sample_train_data, predictors_names, parallel = FALSE)

  # saveRDS(interactions, file = here("tmp", "interactions_lmk_new.rds"))

  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking importances...")

  importance_firm <- check_importance(best_fit, sample_train_data, sample_testing_data, predictors_names,
    type = "firm", parallel = TRUE
  )
  importance_firm <- ggplot2::ggplot_build(importance_firm)$plot$data

  # saveRDS(importance_firm, file = here("tmp", "importance_firm_lmk_new.rds"))

  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking permutation importances...")

  importance_perm <- check_importance(best_fit, sample_train_data, sample_testing_data, predictors_names,
  ...
  importance_perm <- tibble::as_tibble(t(importance_perm)) %>%
    dplyr::select(dplyr::all_of(predictors_names)) %>%
    tidyr::pivot_longer(dplyr::everything(), names_to = "Variable", values_to = "Importance")

  # saveRDS(importance_perm, file = here("tmp", "importance_perm_lmk_new.rds"))

  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking SHAP importances...")

  trdata <- sample_train_data
  ttdata <- sample_testing_data[, predictors_names]
  importance_shap <- check_importance(best_fit, trdata,
    ttdata, predictors_names,
    type = "shap", nsim = 100, parallel = TRUE
  )
  importance_shap <- ggplot2::ggplot_build(importance_shap)$plot$data

  # saveRDS(importance_shap, file = here("tmp", "importance_shap_lmk_new.rds"))

  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking SHAP explanations...")

  shap_fastshap_all_test <- shap_explain(best_fit, ttdata,
    ttdata, predictors_names,
    nsim = 100, parallel = TRUE
  )

  # saveRDS(shap_fastshap_all_test, file = here("tmp", "shap_fastshap_all_test_lmk_new.rds"))

  # preds_test <- predict(best_fit, ttdata)
  # shap_html_test <- fastshap::force_plot( # obsolete
  #   object = shap_fastshap_all_test, feature_values = ttdata,
...
```
```{r modelbart2, message=FALSE, cache=FALSE}
# A tibble: 10 × 2
...
train_data2 <- train_data %>%
  dplyr::mutate(
    int_tc_rl = time_constraint * regime_landmark,
    ...
    # int_w_rl = window_size * regime_landmark,
...
predictors_names_int <- c(predictors_names, "int_tc_rl", "int_tc_rt")
...
sample_train_data2 <- train_data2 %>%
  dplyr::filter(
    time_constraint %in% c(0, 800, 1500),
    mp_threshold %in% c(0.00, 0.4, 0.6, 0.8), window_size <= 150
  )
...
# pred1 <- predict(best_fit2, testing_data2)$.pred
# pred2 <- predict(best_fit2, train_data2)$.pred
# yardstick::huber_loss_vec(testing_data2$mean, pred1) # 0.3419539
# yardstick::rmse_vec(testing_data2$mean, pred1) # 1.045362 (2-3%)
# yardstick::rsq_vec(testing_data2$mean, pred1) # 0.9934289
# yardstick::huber_loss_vec(train_data2$mean, pred2) # 0.2793132
# yardstick::rmse_vec(train_data2$mean, pred2) # 0.9009143
# yardstick::rsq_vec(train_data2$mean, pred2) # 0.9951396
...
if (file.exists(here("output", "importances2_lmk.rds"))) {
  ...
  # split2 <- all_data2 %>% filter(time_constraint %in% c(0, 800, 1500), mp_threshold %in% c(0.00, 0.4, 0.6, 0.8), window_size <= 150)
  # set.seed(19746)
  # sample_data2 <- dplyr::slice_sample(split2, n = 3, by = 1:4)


  interactions2 <- check_interactions(best_fit2, sample_train_data2, predictors_names, parallel = FALSE)

  # saveRDS(interactions2, file = here("tmp", "interactions2_lmk_new.rds"))
  # interactions2 <- readRDS(here("tmp", "interactions2xxx.rds"))
  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking importances...")
  importance_firm2 <- check_importance(best_fit2, sample_train_data2, sample_testing_data2, predictors_names,
    type = "firm", parallel = TRUE
  )
  importance_firm2 <- ggplot2::ggplot_build(importance_firm2)$plot$data

  # saveRDS(importance_firm2, file = here("tmp", "importance_firm2_lmk_new.rds"))
  # importance_firm2 <- readRDS(here("tmp", "importancefirm2xxx.rds"))
  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking permutation importances...")
  importance_perm2 <- check_importance(best_fit2, sample_train_data2, sample_testing_data2, predictors_names,
    type = "permute",
    nsim = 100, parallel = TRUE
  )
  ...
  # saveRDS(importance_perm2, file = here("tmp", "importance_perm2_lmk_new.rds"))

  # importance_perm2 <- readRDS(here("tmp", "importanceperm2xxx.rds"))
  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking SHAP importances...")

  trdata2 <- sample_train_data2
  ttdata2 <- sample_testing_data2[, predictors_names]
  importance_shap2 <- check_importance(best_fit2, trdata2,
    ttdata2, predictors_names,
    type = "shap", nsim = 100, parallel = TRUE
  )
  importance_shap2 <- ggplot2::ggplot_build(importance_shap2)$plot$data

  # saveRDS(importance_shap2, file = here("tmp", "importance_shap2_lmk_new.rds"))

  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking SHAP explanations...")

  ttdata2 <- sample_testing_data2[, predictors_names_int]

  shap_fastshap_all_test2 <- shap_explain(best_fit2, ttdata2,
    ttdata2, predictors_names_int,
    nsim = 100, parallel = TRUE
  )

  # saveRDS(shap_fastshap_all_test2, file = here("tmp", "shap_fastshap_all_test2_lmk_new.rds"))
  # shap_fastshap_all_test2 <- readRDS(here("tmp", "shapfastshapalltest2xxx.rds"))

  # preds_test2 <- predict(best_fit2, ttdata2)
  # shap_html_test2 <- fastshap::force_plot(
  #   object = shap_fastshap_all_test2,
  #   feature_values = ttdata2[, predictors_names],
  #   baseline = mean(preds_test2$.pred), display = "html"
  # )
  # shap_html_test2 <- stringr::str_remove(shap_html_test2, "<meta.+?>")
  # saveRDS(shap_html_test2, file = here("tmp", "shaphtmltest2xxx.rds"))
  # saveRDS(shap_html_test2, file = here("tmp", "shap_html_test2_lmk_new.rds"))
  # shap_html_test2 <- readRDS(here("tmp", "shaphtmltest2xxx.rds"))
...
After evaluating the interactions, we can then perform the analysis of the variable importance. The goal is to understand
how the FLOSS score behaves when we change the parameters.
...
The first line shows the original fit, and the second line shows the refit, taking into account the interactions.

Interestingly, the three methods agree with each other, and the interactions didn't change the overall importance of
the variables. This can probably be explained by the magnitude of the x-axis, which may lead us to overestimate the
importance of the interactions in this model since there is no formal threshold for saying that an interaction is
important or not.
...
#|  (Fig. \\@ref(fig:interaction)). Notice that all methods agree with each other, and the interactions
...
more evident difference is the shape of the effect of `time_constraint` which we will delve carefully below.
Notice however that the refitted model produces a cleaner plot in Fig. \@ref(fig:importanceshap2).

Based on Figures \@ref(fig:importance) and \@ref(fig:importanceshap2) we can infer that:

- **`regime_landmark`**: is the "second" most important feature, however it shows a clear interaction with
  the `regime_threshold` if we look at the color scale, so both must be tuned simultaneously.
  In both plots, it seems to improve the model score when landmark is <5.0s.

- **`regime_threshold`**: seems not as important as the `regime_landmark,` but since there is a high interaction, it must
  not be underestimated, and by prior knowledge we understand that there must be _some kind_ of threshold to mark a regime change.

- **`window_size`**: has a low importance. Nevertheless, it seems to have an optimal value, around 80-120
  In this setting, the model is being tuned towards the changes from atrial fibrillation/non-fibrillation;
  thus, the "shape of interest" is small compared to the whole heartbeat waveform. As Beyer _et al._ noted,
  "as dimensionality increases, the distance to the nearest data point approaches the distance to the
  farthest data point" [@Beyer1999], which means that the bigger the window size, the smaller will be the contrast
  between different regimesm, thus the final objective of the model may influence the optimal window size.

- **`mp_threshold`**: has the lowest impact on the score, and the plot shows that this parameter has virtually no
...
- **`time_constraint`**: is a parameter that must be interpreted cautiously.  The 0 (zero) value means **no
  constraint**, which is equivalent to the size of the FLOSS history buffer (in our setting, 5000). This allows
  us to understand that the model doesn't need this paramenter, and further adjusts in performance can be achieved
  by setting the size of the history buffer.


In short, for the MP computation (the most expensive in terms of complexity), the parameter that is worth tuning
is only the `window_size`, while for the FLOSS computation, both `regime_threshold` (mainly) and `regime_landmark`
shall be tuned.

```{r importanceshap, message=FALSE, warning=FALSE, fig.height = 6, fig.width= 10, out.width="100%"}
#| fig.cap="This shows the Shapley value distribution for each variable. Smaller values means a better FLOSS score
#|  since \"zero\" is a perfect prediction. This plot doesn't take into account the variable interactions."
layout <- "
AABB
CCDD
#EE#
...
all1 <- ((a1 + a3 + a4 + a5 + plot_layout(guides = "collect")) + (a2 + plot_layout(guides = "collect")))
all1 + plot_layout(design = layout) + plot_annotation(title = "Shapley value vs. variable values", theme = ggplot2::theme_bw())
```

```{r importanceshap2, message=FALSE, warning=FALSE, fig.height = 6, fig.width= 10, out.width="100%"}
#| fig.cap="This shows the Shapley value distribution for each variable. Smaller values means a better FLOSS score
#|  since \"zero\" is a perfect prediction. This plot is taking into account the variable interactions, which appears
#|  to result in a cleaner plot."
...
ttdata2 <- sample_testing_data2[, predictors_names]
base_data <- sample_testing_data2[, outcome_name]$mean

d1 <- shapviz::shapviz(shap_fastshap_all_test2[, predictors_names], X = ttdata2, baseline = mean(base_data))
a1 <- shapviz::sv_dependence(d1, "window_size", color_var = "regime_threshold", alpha = 0.05) +
  ggplot2::geom_smooth(method = loess, colour = "#0000ff44", alpha = 0.2) +
  ggplot2::labs(y = ggplot2::element_blank()) +
  ggplot2::theme_bw()
...
all2 <- ((a1 + a3 + a4 + a5 + plot_layout(guides = "collect")) + (a2 + plot_layout(guides = "collect")))
...
```{r global, eval=TRUE, warning=FALSE, fig.height=5, fig.width=10, out.width="100%", cache=FALSE}
...
```{r globalmodel, eval=TRUE, warning=FALSE, fig.height=5, fig.width=10, out.width="100%", cache=FALSE}
...
if (file.exists(here("output", "scores_stats_model_2.rds"))) {
  scores_stats_model <- readRDS(here("output", "scores_stats_model_2.rds"))
} else {
  ...
  saveRDS(scores_stats_model, file = here("output", "scores_stats_model_2.rds"))
}
...
record to the best record.  The top model is the one with the lowest mean across the scores.  The red line indicates
the median score.  The scores above 3 are squished in the plot and colored according to
