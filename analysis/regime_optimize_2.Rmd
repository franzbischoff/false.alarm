...
# pred1 <- predict(best_fit, all_data)$.pred
# yardstick::huber_loss_vec(all_data$mean, pred1)
# yardstick::huber_loss_vec(testing_data$mean, pred2)
# yardstick::huber_loss_vec(train_data$mean, pred)
# yardstick::rmse_vec(all_data$mean, pred3)
# yardstick::rmse_vec(testing_data$mean, pred2)
# yardstick::rmse_vec(train_data$mean, pred)
...
if (file.exists(here("output", "importances_lmk.rds"))) {
  ...
  # source(here::here("scripts", "helpers", "interactions.R"))
  # interactions <- vint(best_fit$fit$fit,
  #   type = "regression",
  #   feature_names = predictors_names, n_jobs = 6, data = train_data
  # )
} else {
  ...
  split <- all_data %>% filter(time_constraint %in% c(0, 800, 1500), mp_threshold %in% c(0.00, 0.4, 0.6, 0.8), window_size <= 150)
  set.seed(19746)
  sample_data <- dplyr::slice_sample(split, n = 3, by = 1:4)
  interactions <- check_interactions(best_fit, sample_data, predictors_names, parallel = FALSE)

  # saveRDS(interactions, file = here("tmp", "interactions_lmk_new.rds"))

  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking importances...")

  importance_firm <- check_importance(best_fit, sample_data, sample_data, predictors_names,
  ...
  importance_perm <- check_importance(best_fit, sample_data, sample_data, predictors_names,
...
predictors_names_int <- c(predictors_names, "int_rt_rl", "int_mp_rl", "int_w_rl")
...
all_data2 <- dplyr::bind_rows(train_data2, testing_data2)
# pred2 <- predict(best_fit2, all_data2)$.pred
# yardstick::huber_loss_vec(all_data2$mean, pred2)
# yardstick::huber_loss_vec(testing_data$mean, pred2)
# yardstick::huber_loss_vec(train_data$mean, pred)
# yardstick::rmse_vec(all_data$mean, pred3)
# yardstick::rmse_vec(testing_data$mean, pred2)
...
if (file.exists(here("output", "importances2_lmk.rds"))) {
  ...
  interactions2 <- check_interactions(best_fit2, sample_data2, predictors_names, parallel = FALSE)

  saveRDS(interactions2, file = here("tmp", "interactions2_lmk_new.rds"))
  # interactions2 <- readRDS(here("tmp", "interactions2xxx.rds"))
  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking importances...")
  importance_firm2 <- check_importance(best_fit2, sample_data2, sample_data2, predictors_names, type = "firm", parallel = TRUE)
  ...
  cli::cli_alert_info("{round.POSIXt(Sys.time())} - Checking permutation importances...")
  importance_perm2 <- check_importance(best_fit2, sample_data2, sample_data2, predictors_names_int,
    type = "permute",
    nsim = 100, parallel = TRUE
  )
  ...
  trdata2 <- sample_data2[, predictors_names_int]
  ttdata2 <- sample_data2[, predictors_names_int]
