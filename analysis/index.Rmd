---
title: "Home"
author: "Francisco Bischoff"
date: "on `r format(Sys.time(), '%B %d, %Y')`"
site: workflowr::wflow_site
output:
  bookdown::html_document2:
    base_format: workflowr::wflow_html
    toc: false
    number_sections: false
editor_options:
  chunk_output_type: console
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, fig.align = "center", dev = "svg")
library(here)
# knitr::opts_knit$set(root.dir = here("docs"), base.dir = here("docs"))
```

<!-- badges init -->

[![minimal R version](https://img.shields.io/badge/R%3E=-4.2.0-brightgreen.svg)](https://cran.r-project.org/)
[![Licence](https://mirrors.creativecommons.org/presskit/buttons/80x15/svg/by-nc-sa.svg)](http://creativecommons.org/licenses/by-nc-sa/4.0/)

Dataset hosted by:

[![](https://zenodo.org/badge/DOI/10.5281/zenodo.4634013.svg)](https://doi.org/10.5281/zenodo.4634013)
[![](https://zenodo.org/badge/DOI/10.5281/zenodo.6879232.svg)](https://doi.org/10.5281/zenodo.6879232)


<!-- badges end -->

## TLDR;

This website contains all the necessary for reproducibility of this Ph.D. It is built joining three frameworks:

-   [workflowr](https://jdblischak.github.io/workflowr/), the main infrastructure that organizes
    every analysis step as Github versions. But instead of showing all commits, it just keeps the
    important ones.
-   [targets](https://docs.ropensci.org/targets/), a
    [Make](https://www.gnu.org/software/make/)-like pipeline toolkit for Statistics and data science
    in R. It supersedes [drake](https://books.ropensci.org/drake/) package. This allows to keep
    track of every step of the data science workflow and helps understand what needs to be rebuilt
    and what has not changed since the last iteration. Every complete analysis shall be registered
    by one entry in the previous package ([workflowr](https://jdblischak.github.io/workflowr/))
-   [thesisdown](https://ismayc.github.io/thesisdown_book/), inspired by the package
    [bookdown](https://github.com/rstudio/bookdown) helps write the thesis in rmarkdown that is
    easily compiled in PDF, Doc, EPUB, and allows to change the theme/format for several
    universities/journals.

Use the big button above ![](figure/workflowr.png) to navigate through the older commits.

Use the top menu for further information and the thesis itself.

Last Targets Run network:

```{r network, echo=FALSE, fig.width=10}
library(magrittr)
network <- readRDS(here("output/network.rds"))
network <- network |>
  visNetwork::visPhysics(hierarchicalRepulsion = list(
    springLength = 1,
    avoidOverlap = 0.5,
    nodeDistance = 120
  ))
visNetwork::visInteraction(network, hover = TRUE, multiselect = TRUE, tooltipDelay = 100)
```
