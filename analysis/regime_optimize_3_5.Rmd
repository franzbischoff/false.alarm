---
title: "Regime changes 3.5 - malignantventricular with PR"
...
if (!file.exists(here("output", "regime_outputs_mvds_pr.rds"))) {
  ...
  saveRDS(all_scores, file = here("output", "regime_outputs_mvds_pr.rds"))
} else {
  all_scores <- readRDS(here::here("output", "regime_outputs_mvds_pr.rds"))
}
```
...
trained_model <- NULL
# Caching ===========
if (file.exists(here("output", "dbarts_fitted_mvds_pr.rds"))) {
  trained_model <- readRDS(here("output", "dbarts_fitted_mvds_pr.rds"))
} else {
  trained_model <- train_models(tree_data, parallel = TRUE, v = 5, rep = 1, grid = 30)
  saveRDS(trained_model, file = here("output", "dbarts_fitted_mvds_pr.rds"))
}
...
# Caching ===========
if (file.exists(here("output", "importances_mvds_pr.rds"))) {
  interactions <- readRDS(here("output", "importances_mvds_pr.rds"))
  ...
  ), file = here("output", "importances_mvds_pr.rds"))
}
```
...
scores_stats %>%
  ...
  ggplot2::theme_bw() +
  ggplot2::labs(
    title = "Scores by recording (lower is better)",
...
[Shiny app](https://franzbischoff.shinyapps.io/FLOSS_ventricular/) (default Score).
