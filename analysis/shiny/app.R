...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      sliderInput(
        inputId = "window_size", label = "window_size:", min = 100, max = 300, value = 200, step = 25
      ),
      sliderInput(
        inputId = "regime_threshold", label = "regime_threshold:", min = 0.2, max = 0.8, value = 0.5, step = 0.1
      ),
      sliderInput(
        inputId = "time_constraint", label = "time_constraint:", min = 800, max = 2000, value = 950, step = 50
      ),
      sliderInput(
        inputId = "mp_threshold", label = "mp_threshold:", min = 0.0, max = 0.9, value = 0, step = 0.1
      ),
      selectInput(
...
server <- function(input, output) {
  ...
  observeEvent(input$time_constraint, {
    if (!(input$time_constraint %in% ecgRecord()$valid_inputs$time_constraint)) {
      val <- which.min(abs(ecgRecord()$valid_inputs$time_constraint - input$time_constraint))
      updateSliderInput(
        inputId = "time_constraint",
        value = ecgRecord()$valid_inputs$time_constraint[val]
      )
    }
  })

  observeEvent(input$mp_threshold, {
    if (!(input$mp_threshold %in% ecgRecord()$valid_inputs$mp_threshold)) {
      val <- which.min(abs(ecgRecord()$valid_inputs$mp_threshold - input$mp_threshold))
      updateSliderInput(
        inputId = "mp_threshold",
        value = ecgRecord()$valid_inputs$mp_threshold[val]
      )
    }
  })
  ...
  ecgRecord <- eventReactive(input$filename, {
    ...
    valid_inputs <- list(
      ...
      time_constraint = unique(res$time_constraint),
      mp_threshold = unique(res$mp_threshold)
    )
