---
title: "Regime changes 4.0 - vtachyarrhythmias dataset"
...
trained_model <- NULL
# Caching ===========
if (file.exists(here("output", "dbarts_fitted_vtds.rds"))) {
  ...
  saveRDS(trained_model, file = here("output", "dbarts_fitted_vtds.rds"))
}
...
# Caching ===========
if (file.exists(here("output", "importances_vtds.rds"))) {
  interactions <- readRDS(here("output", "importances_vtds.rds"))
  ...
  ), file = here("output", "importances_vtds.rds"))
}
```
...
#|  overall).  The recordings are sorted (left-right) by the median (best) score achieved in descending
#|  order.  The blue color highlights recordings where models had an IQR variability of less than 10.
#|  As a simple example, a recording with just one regime change, and the model predicts exactly one change,
#|  10s far from the truth, the score will be roughly 1."
...
scores_stats %>%
  ...
  ggplot2::ggplot(ggplot2::aes(
    x = reorder(reorder(id, -min), -median),
...
[Shiny app](https://franzbischoff.shinyapps.io/FLOSS_vtachy/).
...
#|  sorted by the SD and IQR, respectively.  The bluish colors highlights models with an SD below 10
#|  and IQR below 10."

if (file.exists(here("output", "scores_stats_model_4.rds"))) {
  scores_stats_model <- readRDS(here("output", "scores_stats_model_4.rds"))
} else {
  ...
  saveRDS(scores_stats_model, file = here("output", "scores_stats_model_4.rds"))
}
...
model_mean_best <- scores_stats_model %>%
  ...
  ggplot2::labs(subtitle = "Ordered by Mean and SD", colour = "SD < 10", x = ggplot2::element_blank(), y = "Score distribution")
...
model_median_best <- scores_stats_model %>%
  ...
  ggplot2::theme_bw() +
  ggplot2::theme(axis.text.x = element_text(size = 8, angle = 90, vjust = 0.5, hjust = 1)) +
  ggplot2::labs(subtitle = "Ordered by Median and IQR", colour = "IQR < 10", x = "Model ID", y = "Score distribution")
