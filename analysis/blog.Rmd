...
# library(kableExtra)
# library(targets)
# knitr::opts_knit$set(root.dir = here("docs"))
...
4. **complexity**: This is the SQI defined by Batista, *et al.* [@Batista2014], and we will write it
   ...
   signals. Fig. \@ref(fig:cplxty) shows this intuition. The formula is just the "sum of squares
   of the differences".
...
to `a165l`. By manual inspection, we have the following files as _negative_ for noise #4, #7, #8,
#10, #11, #12. And for _positive_ for noise the following #1, #2, #3, #5, #6, #9.
...
if (file.exists(here::here("output/backtofilters.rds"))) {
  backtofilters <- readRDS(here::here("output/backtofilters.rds"))
  ...
  filenames <- find_all_files(classes = "asystole", limit_per_class = length(neg) + length(pos))
  ...
  saveRDS(list(neg_files = neg_files, pos_files = pos_files, neg_filters = neg_filters, pos_filters = pos_filters),
    file = here::here("output/backtofilters.rds"), compress = "xz"
  )
}
gc()
```

Fig. \@ref(fig:filters-samples) shows a sample of a positive and a negative signal (for noise)
and a sample of some SQI. We need an index with a low value when the signal has low noise and a high
value when there is a noisy signal (that we will not process).

```{r filters-samples, fig.cap = "ECG samples of the positive (for noise) and negative", warning=FALSE}
lwy <- 0.2
size <- 9
...
As we can see in Fig. \@ref(fig:filters-samples), the SQI fluctuates where there is a clean QRS
complex. The threshold must be above this fluctuation.

To be more robust in defining such threshold $\theta$, instead of finding the maximum value, we will
take the quantile 0.9 of those values and then multiply this value by some constant:
...
without normalization and without gain).

It is important to say that the normalization step is done when computing the Matrix Profile. Thus,
...
implications on comparing Mobility, Complexity [@DelRio2011], and Kurtosis because they are
...
gain $g$ applied directly on the data or on the SQI itself.
