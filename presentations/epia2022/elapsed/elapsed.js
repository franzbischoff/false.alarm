var ElapsedTimeBar = {
  ...
  loop() {
    ...
    if (Reveal.getPlugin("menu").isOpen()) {
      this.pause();
    }
...
Reveal.addEventListener('overviewshown', () => ElapsedTimeBar.pause());
Reveal.addEventListener('overviewhidden', () => ElapsedTimeBar.resume());
Reveal.addEventListener('menu-ready', function (event) {
  document.querySelector("body > div.slide-menu-wrapper").addEventListener('mouseleave', () => {
    ElapsedTimeBar.resume();
  });
});
